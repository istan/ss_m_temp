Ext.define "stable_mobile.controller.health.IndexViewController",
	...
	onMedicationBtn: ->
		...
		horseId = horse.id
		console.log horseId
		if SS.cache.horses[horseId]? && SS.cache.horses[horseId].prescriptions?
		...
		if navView
			navView.push(SS.addCmp "#medciations", "stable_mobile.view.health.medication.Index", 
				horseId: horse.id
	...
	onVaccinationBtn: ->
		...
		horse = @getView().getData()
		if navView
			navView.push(SS.addCmp "#vaccinations", "stable_mobile.view.health.vaccination.Index", 
				horseId: horse.id
			)
